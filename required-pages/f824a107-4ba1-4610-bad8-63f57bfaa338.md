...
| **Feeds** | Pulls structured data from external feeds (earthquake catalogs, alert feeds, any JSON/GeoJSON endpoint) on a schedule and displays the latest records on your pages — without creating one page per record |
| **Elasticsearch** | Connects to an Elasticsearch/sist2 instance for full-text asset search across large document collections |
/%

Every one of these is **disabled by default**. They ship with the platform but do nothing until an administrator enables them, so a site that enables none of them runs exactly the same core platform.
