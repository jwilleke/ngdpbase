...
__Location:__ project root (`.env`) — gitignored, never committed
__Purpose:__ tells `server.sh` and PM2 where to find data before the app starts
__Template:__ `.env.example` in the project root
...
| __Fast__ | `FAST_STORAGE` | Sessions, logs, users, search index, config, `.install-complete`, `page-index.json` |
| __Slow__ | `SLOW_STORAGE` | Pages (content), attachments, backups |
/%
