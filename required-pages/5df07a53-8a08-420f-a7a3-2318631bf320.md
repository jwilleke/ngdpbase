...
The admin account keeps the bootstrap password from `NGDPBASE_ADMIN_PASSWORD`. Change it immediately after first login. ngdpbase ships no default admin password: a headless install with that variable unset refuses to start rather than creating an account whose credentials are published in the repository.
