...
Installation is considered __required__ if any of the following are true:
...
There is no step for pages. Every time the platform starts, it adds the documentation, help and system pages it ships that this site has never had, so they are already in place when the wizard opens. See [Startup Pages].
...
%%table-striped
|| Step || What it does ||
| __writeConfig__ | Writes `${FAST_STORAGE}/config/app-custom-config.json` with the application name, base URL, session secret, and organization details. A random 32-byte session secret is generated if not provided. |
| __writeOrganization__ | Creates `${FAST_STORAGE}/users/organizations.json` with a Schema.org `Organization` object using the organization details from the form. |
| __updateAdminPassword__ | Sets the admin user's password to the value provided in the form. The `admin` user is created at startup using `NGDPBASE_ADMIN_PASSWORD` — this step replaces that bootstrap password. |
| __markComplete__ | Creates `${FAST_STORAGE}/.install-complete` with a JSON marker containing the completion timestamp and version. |
/%
...
To reset a partial installation and start over from scratch, click the __Reset Installation__ link on the install page. This:
...
4. Backs up and removes the pages in the pages directory, and the record of which startup pages this site has had, so they are added again at the next start
...
__Location:__ `${FAST_STORAGE}/.install-complete`
__Format:__ JSON
...
__Do not delete this file__ on a running instance — the server will show the install wizard to all visitors and overwrite `app-custom-config.json` if the form is submitted again.
