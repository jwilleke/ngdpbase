...
Some plugin parameters accept **special values** that [{$applicationname}] resolves for you at render time — so one line of markup works on any page, for any user, without editing. This page is the single reference; individual plugin pages link here rather than repeating it.

> **Shortcuts vs [Contextual Variables]:** these are two different things.
>
> - A **contextual variable** like `${pagename}` or `${pageslug}` expands *anywhere* in a page (text or a plugin parameter) to a value — see [Contextual Variables].
...
When you deliberately want the **slug** form regardless of tagging — for example matching only slug-tagged media — use the `${pageslug}` contextual variable instead of `current`:
