...
> __Vocabulary model note (2026-07):__ system keywords are the __machine/automation bucket__ — terms written by automation rather than typed by people. Today that means:
>
> - __`capture`__ — provenance mark on pages created by the capture bookmarklet.
> - Future: AI-suggested subject classification (auto-tagging).
>
> Lifecycle terms (`draft` / `review` / `published`) are __no longer keywords__ — editorial state lives in the single-valued `status:` frontmatter field (absent = published), set via the editor's Status select.
