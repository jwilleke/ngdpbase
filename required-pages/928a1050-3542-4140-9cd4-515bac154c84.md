...
| `ngdpbase.managers.addons-manager.addons-path` | `"./addons"` | Where add-ons are discovered. Accepts a single string or an **array** of entries. A plain entry is a directory; an entry of the form `node_modules:<glob>` (e.g. `node_modules:@jwilleke/*-addon`) discovers add-ons installed as npm packages |
...
| `ngdpbase.addons.page-reseed` | `false` | Refresh add-on-seeded pages from source at boot when the source changed **and** the page was never edited in-app. Operator-edited pages are always skipped |
/%
