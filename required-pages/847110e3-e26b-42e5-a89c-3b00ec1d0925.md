...
Available adapters today: __geojson__ (GeoJSON FeatureCollections), __rest-json__ (generic JSON endpoints), and __xml__ (generic XML documents — attributes are addressed as `element.@attribute`, element text as `element.#text`). More formats (RSS/Atom, CSV) are planned.
...
- __One rule per plugin call.__ To filter on two different fields, use two separate `[{DataFeed}]` tags.
- __A malformed rule filters nothing__ rather than showing an error — so if `exclude=` seems to be doing nothing, check the `field~pattern` spelling first. There must be a `~`, with a field name before it and a pattern after it.
...
Two situations are __not__ treated as an empty feed, and `empty=` deliberately does not cover them:

- __No `source` given__ — the plugin call is incomplete.
- __The feeds addon is not enabled__ — nothing was fetched at all.
