...
The __System Category__ classifies a page by its role in [{$applicationname}]. It controls where the page is stored and how the system treats it.

Only __administrators__ can change the System Category. Non-admin users see it as a read-only field on the edit form.
...
Pages with a `required` storage location are managed as __required pages__ — they are seeded from the application source and can be resynced by an admin from Admin › Required Pages Sync. Changes saved to a required page on the platform will be overwritten on the next sync.
