...
## Gap 3: authentication — UNDECIDED

Every existing `ngdpbase` API route either relies on an authenticated wiki session (cookie) or Authentik forward-auth at the ingress (`/api/page/ingest`, see `#819`). Neither works for Dawarich: its Immich client is a bare server-to-server HTTP call sending only an `x-api-key` header (the same convention Immich itself uses) — no session, no OAuth bearer flow, no interactive login.

No decision yet on how to gate these two routes. One option, not yet agreed: a single shared-secret API key, config-driven (`ngdpbase.dawarichCompat.apiKey`, set via `app-custom-config.json` or env, analogous to how the existing `ngdpbase-ingest-creds` SOPS secret feeds credentials into `geohazardwatch`/`jimsmcp` today), checked via `req.headers['x-api-key']` with a constant-time comparison — mirroring Immich's own convention, which is also what Dawarich's client already sends unprompted. There's no existing API-key-checking middleware in `ngdpbase` to reuse as-is (the closest analog, `ShareManager`'s token-gate for `/share/:token/*`, is a URL-embedded token for anonymous browser access, a different shape than a header-based service credential — though the *pattern* of `shareGate()`, one function checked first in every handler, is worth reusing regardless of what the check ends up being).

**This needs a decision before implementation starts** — flagging it here rather than picking for you.
