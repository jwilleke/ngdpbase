...
- Testing:
  ...
  - playwright E2E: 72/72 pass on jimstest after restart (chromium-maintenance suite end-to-end exercises admin auth)
  - Per-install propagation verified: HTTP 403 on `/admin/organizations` on all four ports
- Commits: 4a95a086 (`fix(#622): bump testTimeout to 30s`), f9680431 (`fix(#623): consistent "Back to Dashboard" button across all admin pages`), 748a4586 (`fix(#624): rewire /admin/organizations onto OrganizationManager + build out Edit modal`), 4f8868e8 (`chore: release v3.5.2`)
- Files Modified:
  - vitest.config.ts (testTimeout 20000 → 30000)
