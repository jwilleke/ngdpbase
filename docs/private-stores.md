...
- __`POST /private-store/unlock`__: the same unlock after a server restart, which drops the bag while the session cookie survives. Reuses the session's handle if it has one, mints one if not (kept only once an unlock succeeds). Takes either the password or, as `words`, the 12 recovery words ([#1453](https://github.com/jwilleke/ngdpbase/issues/1453)): `unlockPrivateStoresWithMnemonic()` unwraps the same KEK through the envelope's recovery wrap, and both paths share `unlockWithKek()` for everything after. Both ask the sign-in throttle first and record a failure the same way (`secretCheckBlocked`, `recordFailedSecret`); the words are never logged or recorded.
- __`POST /recover-password`__ ([#1452](https://github.com/jwilleke/ngdpbase/issues/1452)): a forgotten password, reset with the 12 words. Open to anyone, like the sign-in form. `UserManager.resetPasswordWithRecoveryWords` refuses an unknown, inactive or external account; `resetPasswordWrapWithMnemonic()` unwraps the KEK with the words and replaces only the password wrap (`rewrapPasswordWithKek` — the KEK and the recovery wrap are unchanged, so every store's DEK wrap stays valid); then `updateUser` sets the sign-in password, which records the change. Every failure gets one answer, counts against the sign-in throttle, and a throttled caller is refused even with the right words. Nothing is unlocked: the person signs in with the new password afterwards.
