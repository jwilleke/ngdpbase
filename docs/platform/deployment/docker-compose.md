...
Then add `HEADLESS_INSTALL=true` to the environment (`.env`, or directly under `environment:` in the compose file). Headless install creates the default `admin` / `admin123` user — change the password on first login.
