...
8. ⏳ **Adapters by dependency cost** — ✅ `rest-json` (zero-dep) DONE (generic REST GET → JSON; `itemsPath` dot-path or envelope auto-detect; shares `buildRecord` with geojson). Remaining, **driver + dependency sign-off gated**: `rss-atom` (+XML parser), `csv` (trivial or small dep), `xls` (+spreadsheet lib) — each a separate PR calling out its dependency. No driver for these today (USGS = geojson; generic REST = rest-json cover the open geohazardwatch issues).
