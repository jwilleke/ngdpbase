...
- The editor `/save` path is outside the NCM funnel, and a save writes exactly what was typed: no fix steps, no normalizer (decision 2026-09-12 on [#1332](https://github.com/jwilleke/ngdpbase/issues/1332); saves must stay fast, [#1333](https://github.com/jwilleke/ngdpbase/issues/1333)). JSPWiki syntax and every other conversion happen only in the funnel. `funnelCoverage.test.ts` fails if `PageManager.savePage` calls them.
- A new ingestion path must go through the normalizer and be added to `converters/ncm/__tests__/funnelCoverage.test.ts`.
- Conversion has one door with two halves on `PageManager` (#1486): `convertPageToNcm` (pure) and `completeNcmConversion` (image localization, footnote transfer). Every path takes both; none reaches a side effect around them (§3.5).
...
| __Footnotes__ | `[^id]` reference + `[^id]: text` definition (single- and multi-paragraph). __Conversion transfers definitions to the sidecar footnote list__ ([#1125](https://github.com/jwilleke/ngdpbase/issues/1125)): the convert-existing path moves `[^id]: text` into `FootnoteManager` records (id preserved verbatim — refs must keep resolving), leaves the refs in the body, and appends `[{FootnotesPlugin}]` when absent. A colliding sidecar id keeps the body definition and warns. The extraction (`converters/ncm/footnotes.ts`) is pure; the sidecar write happens in `PageManager.completeNcmConversion` through `FootnoteManager.transferFromContent`, on every path, when the page has a uuid (#1486). | `FootnoteManager`, `FootnotesPlugin`, `MarkupParser` footnote extraction |
| __Embedded images__ | `![alt](attachment-ref)` — image downloaded and stored as an attachment; the source URL / `data:` URI is replaced with the local attachment ref. Remote images are localized by Convert to NCM only (preview shows it, apply stores it); file import, ingest and MCP leave them as links today. A file import's own image files are uploaded by the importer | `AttachmentManager.localizeRemoteImages`, `ImportManager.importPageAttachments()` |
...
When a conversion localizes images (Convert to NCM, the only path that asks for it today — see §3.5), for every embedded image whose source is a remote URL or a `data:` URI. The code is `AttachmentManager.localizeRemoteImages`, reached through `PageManager.completeNcmConversion`:
...
- Steps convert JSPWiki syntax that is never Markdown (`{{{ }}}`, `!` headings, `''italic''`, `[{INSERT}]`, `;:`, bare `%%` closers, `**` bullets), remove the JSPWiki "More Information" footer, and bring valid Markdown to the house style (`-` markers, tight lists). They run only in the NCM funnel, never on an ordinary save.
- Registry order (`FIX_STEPS`): `jspwiki-code-markers`, `jspwiki-headings`, `jspwiki-italic`, `jspwiki-insert`, `jspwiki-indent`, `style-closers`, `more-information-footer`, `jspwiki-bullets`, `bullet-markers`, `tighten-lists`. Order matters: code first, so later steps see examples as code; headings before the footer step, which looks for a `#` "More Information" heading; bullets before the list steps.
- A construct that is also valid Markdown with another meaning is converted only where the source is known to be JSPWiki, in `JSPWikiConverter`, never in the funnel (#1343). `{{text}}` monospace is the case today (#1342, operator decision 2026-09-26): `{{name}}` is template syntax in Markdown from anywhere else. The converter matches braces balanced, reads `[[`, `&#91;` and `&#124;` inside as the reader saw them, and leaves a span in a table row that would hold a `|`. `;term:def` definitions are mapped there too (`**term**: def`).
...
- `jspwiki-headings` turns a JSPWiki `!` heading into ATX (`!!!` to `#`, `!!` to `##`, `!` to `###`: more marks, larger heading). A Markdown image `![alt](src)` is not a heading. `JSPWikiConverter` calls the same step, so file import and the funnel read `!` lines one way.
- `jspwiki-italic` turns JSPWiki `''italic''` into `*italic*`. Each `''` toggles, and, as in Apache's parser, a run carries across paragraphs and list items, so it is closed and reopened in every block; `''''` is nothing, and spaces (a no-break space too) move outside the marks so Markdown reads them. A page with an odd `''` count is left alone. Never in code spans or `[{Plugin}]` calls. `JSPWikiConverter` calls the same step.
- `jspwiki-insert` rewrites JSPWiki's long plugin form, `[{INSERT Name WHERE a=1}]` (INSERT and WHERE optional) and `[{org.apache.wiki.plugin.Name}]`, to `[{Name a=1}]`; only the head changes. Another package, and an escaped `[[{INSERT …}]` example, stay.
- `jspwiki-indent` turns a JSPWiki `;:text` line (a definition with no term: the indentation idiom) into a plain paragraph, each its own. A `;term:def` definition is the JSPWiki converter's, not this step's.
- `style-closers` turns a bare `%%` that closes a style block into `/%`, the only closer NCM writes (the renderer still tolerates `%%`). A `%%` counts as a closer under Apache's rule: not followed by a letter or `(`, with a block open. An inline opener never closed ends with its paragraph; a block-form opener stays open until closed. A `%%` with nothing open is left alone.
- `more-information-footer` removes the JSPWiki "More Information" footer (heading, the "There might be more information…" sentence, the `ReferringPagesPlugin` call); the Referring Pages tab lists the same pages. Only a section whose every line is a footer line goes; one with anything else under the heading stays for review, and the plugin without the heading is valid NCM.
...
- The door has two halves (#1486). `convertPageToNcm(raw)` is pure. `completeNcmConversion(content, { pageName, uuid }, ctx, { dryRun, localizeImages })` does what writes, after the caller's validation and stale-version checks: it localizes remote images into attachments (AttachmentManager, only when `localizeImages` is set, i.e. interactive Convert to NCM) and moves footnote definitions to the page's list (FootnoteManager). Convert to NCM, ingest, both MCP writes and file import all call it; `funnelCoverage.test.ts` fails if a path reaches either side effect around it.
...
- `scripts/fix-page-markdown.ts` runs the same steps across a page store: the migration of stored pages (#1347). Dry run by default, with a report. `--apply` (server stopped) writes each changed page through `PageManager.savePage` as one version by the system principal, keeping `lastModified`, so the page door reconciles the search index, link graph and rendered cache (#1341). Private pages are reported and never written: a store page is written with its owner's keys, which a migration run does not have. It runs the fix steps only, not `normalizeExistingPageToNcm` or the writing half.

What each path runs, as of #1486:

| | File / URL import | Ingest API | MCP create / update | Convert to NCM (More menu, `/admin/convert`) | Migration script |
|---|---|---|---|---|---|
| Front end | format converter + `normalizeToNcm(content, format)` | — | — | — | — |
| Fix steps | yes | yes | yes | yes | yes |
| Links, tables, `ncmVersion` | yes (format-aware funnel) | yes | yes | yes | no |
| Footnotes to the list | yes | yes | yes | yes | no |
| Remote images to attachments | no | no | no | yes | no |
| Preview first | yes | no | no | yes | dry run |
| Saved as | the importer | the caller | `mcp-server` | the converting user | system principal, `lastModified` kept |
...
__No auto-migration guarantee:__ `PageManager` save/load is untouched by the NCM conversion and the fix steps (§3.5). There is no startup/cron/on-read normalization path. Every NCM invocation is explicit: a single item (import, Convert to NCM apply, ingest, MCP create/update), or the operator-run migration script, which is dry-run by default and writes only with `--apply` on a stopped server (§3.5). (Known accepted caveat: MCP `update_page` with metadata-only still NCM-normalizes that one page's body — explicit-per-page, not bulk.)
