...
- `jspwiki-code-markers` runs first, so later steps see the examples inside a `{{{ }}}` block as code. It leaves alone a `{{{` that opens mid-line and closes on a later line, and a block opened by ```` ``` ```` and closed by `}}}` (those lines are code to the parser); a page with an unclosed `{{{` block is not changed at all.
