...
In `src/` ([#1219](https://github.com/jwilleke/ngdpbase/issues/1219)): `PolicyInformationPoint.canUserAccessPage` decides one; `PolicyInformationPoint.filterAccessiblePages` filters many with the same tiers, over the page index, with no log or audit record per page; `PageManager.listPagesFor(subject, action)` is the door, and `getAllPages()` is the raw index for callers with no reader. `WikiRoutes.listingDoor.test.ts` holds the reader-facing handlers to the door statically, and `PolicyInformationPoint.filter.test.ts` holds the filter to the decider in both directions.
...
- __A full rebuild of a subject still compiles.__ Since [#1212](https://github.com/jwilleke/ngdpbase/issues/1212) `PermissionSubject` requires `username`, `roles` and `isAuthenticated`, and every carried identity type — the request's `userContext`, `WikiContext`, `ParseContext`, the ACL and attachment shapes — declares the same three, so a partial rebuild is a compile error and the #631 job shape says `resolveRolesNow: true` instead of omitting roles. What the type still cannot refuse is a rebuild with all three fields present and no `viaToken` / `viaShare`: both delegations must stay optional, because a session has neither and a dummy grant would be a new lie. That last shape is what `scripts/check-permission-subject.ts` exists for — a convention, not the compiler. Tracked as [security-posture.md](security-posture.md) P1 and [#1179](https://github.com/jwilleke/ngdpbase/issues/1179).
