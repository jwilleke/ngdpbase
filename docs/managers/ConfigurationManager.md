...
This is not a style preference. An empty enum shipped a security control that silently did nothing: `getProperty(key, fallback)` returns the empty string rather than the fallback, because `""` is not `undefined`, so a default derived elsewhere was never reached and a deployment that meant to refuse to boot would not have ([#1118](https://github.com/jwilleke/ngdpbase/issues/1118)). The preset that supplied that default has since been removed ([#1144](https://github.com/jwilleke/ngdpbase/issues/1144)), which makes the rule stronger rather than moot: the shipped value is now the only value there is.

__Nothing supplies a value but the key itself.__ There is no preset layer: no key names a posture and defaults a set of others from it. `ngdpbase.security.profile` did exactly that and was removed ([#1144](https://github.com/jwilleke/ngdpbase/issues/1144)) — a preset that supplies values invisibly is a second source of truth for every key it touches, and telling which settings are explicit and which are implied by a label requires knowing the preset. `baseline`, `hardened` and `regulated` survive as __documented recommendations__ — prose and tables an operator reads and applies themselves — and never as configuration values. A configuration file whose effective values cannot be read off the page is one an operator will get wrong.
