...
**Server needed:** The Fairways instance (the demo URL example below assumes `http://localhost:2121`; substitute the production URL)
...
1. **Site is reachable:** `curl -s -o /dev/null -w "%{http_code}\n" http://localhost:2121/` returns `200` or `302`. If not, ask the host to start it (`./server.sh start` from the install directory).
...
> **About the URLs in this guide:** every example URL uses `http://localhost:2121`. In a real deployment substitute your actual hostname and port. The path portion (`/admin/...`, `/view/...`) is identical regardless of host.
...
> 🗣️ *Talk-track: "As admin you have a master key. Most of what you'll do day-to-day is page editing and member management — the same screens regular editors use. The admin-only screens come up when you're setting things up or troubleshooting."*
...
1. Click **Add user** (or **+ New**).
2. Fill in: username (a short handle — used in URLs), display name (the human name, e.g. "Molly Crotty"), email, initial password, and assign roles.
3. **Roles cheat-sheet:**
...
4. Save. The new member gets their credentials out-of-band (you tell them, or in some setups email is configured to send a welcome).

> ⚠️ *Common pitfall: don't give residents `admin`. The `editor` role is what you usually want for the newsletter editor / board secretary. Reserve `admin` for the technical contact.*
...
> 🗣️ *Talk-track: "There is no separate 'publish' step. Save means it's live. If you want a draft, set Audience to just yourself + admins (Act 4) and flip it open later."*
...
1. Open `/view/Members-Only` → Edit
2. Open the **Audience** dropdown — check `authenticated`. Verify the username typeahead below the dropdown is empty.
3. Save.
...
> ⚠️ *Common pitfall: "Audience: empty" is **open access**, not "nobody". Leave all roles unchecked + no users to make a page world-visible. If you want admins-only, check `admin` and **nothing else**.*
>
...
3. Save. The page is live; **Recent Changes** picks it up automatically.

> 🗣️ *Talk-track: "Most HOA sites post news as a single rolling page with newest-on-top. If you'd rather have one page per announcement, create them as separate pages and link them from a News index — works either way."*
...
> 🗣️ *Talk-track: "The persisted log is the receipt-of-record — even if mail fails, you can see who submitted what. There are honeypot + rate-limit defenses in front of the form so you don't get spammed."*
...
1. Navigate to the page in Window A (admin) and Window B (regular member).
2. **As regular member:** click an open day → reservation form appears → submit a reservation for the clubhouse → see confirmation.
3. **As admin:** open `/admin/events` (or whatever path the calendar admin lives at). See the new reservation, with the member's name and the time block. Approve or reject if approval flow is enabled.

> 🗣️ *Talk-track: "Conflicts are detected at submission time — two members can't double-book the same hour. The admin view lets you override, reschedule, or delete if needed."*

### Member forms

If you have a forms-driven survey or interest form set up (e.g. `/view/Architecture-Request`):

1. Walk through submitting one as a member.
2. As admin, navigate to the form's submissions view to see the responses.
...
> 🗣️ *Talk-track: "Forms are JSON-defined — no programming. You list the fields, pick the types, give it a submission handler, and you have a working form. If you'd prefer a dedicated form for, say, lifeguard signup, that's a 10-minute admin task."*
...
```bash
curl -s http://localhost:2121/api/health | jq .
```
...
> 🗣️ *Talk-track: "Patch releases (3.14.x → 3.14.y) are safe; minor releases (3.14.x → 3.15.0) may add a feature you'll want to read the changelog for. Major releases are rare and announced separately."*
