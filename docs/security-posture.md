...
The decision record for how an ngdpbase instance declares and inspects its security-related settings. How to write a route, manager method or addon that satisfies P1 and P2 is the [security developer guide](guides/security-developer-guide.md).
...
__Surfaces narrowed under this principle so far__ (the compiler does the search — narrow the type, fix what it finds): `hasPermission` ([#1164](https://github.com/jwilleke/ngdpbase/issues/1164)); `PermissionSubject`'s three fields required ([#1212](https://github.com/jwilleke/ngdpbase/issues/1212)); the boot and timer paths ([#1197](https://github.com/jwilleke/ngdpbase/issues/1197), [#1196](https://github.com/jwilleke/ngdpbase/issues/1196)); `ConfigurationManager.setProperty` / `resetToDefaults` and `BackupManager.updateAutoBackupConfig` take an `ActorContext` — `PermissionSubject | JobContext`, in `src/context/ActorContext.ts` — and the request subject now carries `ipAddress`, so a manager records the address from the context it is handed rather than from a second parameter (#1179, first slice; eighteen call sites, thirteen of which had been recording `system`); `UserManager.createUser` / `updateUser` / `deleteUser` / `assignRole` / `removeRole` / `searchUsers` take the same `ActorContext` in place of the optional `AuditActor` bag, so an account write cannot be recorded as `unknown` and an identity provider that provisions an account says so in a `JobContext` reason rather than a literal `'system'` (#1179, second slice; `jobContextFromRequestWithReason` exists for the act inside a request that has no subject of its own yet — self-registration, provisioning, a sign-in refreshing its statistics); `AttachmentManager.uploadAttachment` / `deleteAttachment` / `updateAttachmentMetadata` / `updateAssetMetadata` take the context positionally — it was an optional `options.context` that fell through to a fail-closed check and an `unknown` record — and `ImportOptions.actorContext` is required, so the import door cannot be reached without the importer's own subject (#1179, third slice); `BackupManager.createBackup(ctx, options)` and `AuditManager.restartAuditChain(reason, ctx)` / `BaseAuditProvider.restartChain` take the context — the backup took an optional actor bag, the chain restart a bare name — and `jobContextFromOperator` gives a command-line operator an origin of its own (#1179, fourth slice); `CommentManager.addComment(pageUuid, ctx, content)` / `deleteComment(pageUuid, commentId, ctx)` take the subject and record `comment-create` / `comment-delete` — the first of the rank-E doors, where the record is what makes the context a parameter something reads ([#1232](https://github.com/jwilleke/ngdpbase/issues/1232)); `FootnoteManager`'s five writes — add, import, transfer, update, delete — take the subject and record `footnote-edit` with the action, one record per transfer rather than one per definition, and a dry run records nothing ([#1233](https://github.com/jwilleke/ngdpbase/issues/1233)); `AddonsManager.saveProfileSections(ctx, body)` and the addon hook `saveProfileSection(ctx, body)` hand an addon the subject, not a name — the one place the host itself did the flattening ([#1234](https://github.com/jwilleke/ngdpbase/issues/1234)); `NotificationManager.dismissNotification(id, ctx)` takes the subject for a uniform door — it records nothing, because a dismissal hides a banner from the dismisser and nothing else, and that decision is written here so the door is not mistaken for a boundary ([#1235](https://github.com/jwilleke/ngdpbase/issues/1235)); `ImportOptions` carries one identity — the `actor?: string` and `actorIsSystem?` that rode beside `actorContext` are gone, and the run summary's actor, `isSystem` and `origin`, the page's `editor` and its `author` fallback all derive from the context ([#1236](https://github.com/jwilleke/ngdpbase/issues/1236)).
...
Since 2026-09-05 the state of `src/` is: no role-name gate outside the evaluator and the named-user lookups (`src/__tests__/roleNameGates.test.ts` holds it), and no `isAuthenticated` allow in `WikiRoutes.ts` — every former one asks a permission through `permitted()`, and `refuse()` is the one place the flag is read, to choose 401 / a login redirect over 403 (`WikiRoutes.permissionGates.test.ts` holds it). Three permissions were added for the routes that named none: `profile-manage` and `comment-create` on one `Authenticated` policy, and `token-mint` for editors and admins, which a token can never carry as a scope. The picker's people search ([#694](https://github.com/jwilleke/ngdpbase/issues/694)) asks `search-user`, granted on the same `Authenticated` policy, so no session-flag scope choice remains. The views ask the same questions through `can(permission)` from the common template data rather than `currentUser.roles.includes('admin')`; the two reads left are a badge naming a membership and a count of accounts holding a role. `scripts/check-permission-gates.ts` (`npm run lint:gates`, on the pre-commit hook) holds all of `src/`, `addons/` and `views/` there, with every remaining read listed beside its reason.
...
| `VersioningFileProvider.runRetentionPurge(systemContext(…))` | __acts__ | destroys pages past the delete-retention window — `page-delete` action `purge` per page, principal, origin `boot`; the hourly tick passes `scheduleContext(…)` and records the same with origin `schedule` ([#1196](https://github.com/jwilleke/ngdpbase/issues/1196): the purge takes the `JobContext` itself, not a string saying when) |
| `FileAuditProvider.cleanup(systemContext(…))` | __acts__ | removes audit archives past the retention window — `security-event` action `audit-archive-purge` per archive, principal, origin `boot`, written through the provider itself because the manager above it is still initialising; the hourly tick records the same with origin `schedule` (#1196) |
...
| `BackupManager` scheduled backup | __acts__ (on the tick) | the tick calls `createBackup` with an actor built from `scheduleContext(…)` — `backup-create` names the principal, origin `schedule` and the reason instead of `unknown` / `actorMissing` (#1196) |
| `MediaManager` scan timer, `NotificationManager` autosave, `LunrSearchProvider` persist, `LinkParserHandler` refresh, `FileAuditProvider` flush, `AgentTokenManager` maintenance, journal reminders, feeds ingest | wires a timer; the tick acts on derived state, unrecorded by design (#1196) | each tick rewrites what can be rebuilt (indexes, caches, the notification and feed-record stores) or emits nothing an assessor asks "who" about; a `JobContext` nothing reads was not added — the parameter arrives when the tick first records something |
