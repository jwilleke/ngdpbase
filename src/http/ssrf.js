...
const MECHANISM_CIDRS = MECHANISM_DENIED.map((c) => ipaddr.parseCIDR(c));
const DEFAULT_POLICY_CIDRS = DEFAULT_POLICY_DENIED.map((c) => ipaddr.parseCIDR(c));
/**
...
function longestMatch(addr, cidrs) {
    let best = null;
 ...
 */
export function isMechanismDenied(address) {
 ...
 */
export function isAddressAllowed(address, policy) {
    const addr = canonicalise(address);
    if (!addr)
        return false;
    if (longestMatch(addr, MECHANISM_CIDRS) !== null)
        return false;
 ...
 */
export function validateUrl(raw, policy) {
    let url;
    try {
        url = new URL(raw);
    }
...
export function reconcilePolicy(deniedRanges, allowedRanges) {
    const conflicts = [];
    ...
    const malformed = [];
    const duplicates = [];
    for (const [label, ranges] of [['denied-ranges', deniedRanges], ['allowed-ranges', allowedRanges]]) {
        for (const range of ranges) {
            try {
    ...
    for (const range of allowedRanges) {
        ...
        if (hits.length > 0) {
            conflicts.push(`allowed-ranges: '${range}' covers an address range that is never permitted ` +
