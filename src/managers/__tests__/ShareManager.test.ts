 ...
 * - recordAccess()/shutdown(): aggregated share_access audit rows, never per-view
...
describe('ShareManager', () => {
  ...
  describe('issue()', () => {
    ...
    test('audits share_create', async () => {
      const rec = await sm.issue({ kind: 'keyword', keyword: 'k' }, '24h', 'alice');
      const evt = auditEvents.find(e => e.eventType === 'share_create');
  ...
  describe('revoke()', () => {
    test('sets revokedAt, persists it, audits, and is idempotent', async () => {
      ...
      expect(auditEvents.some(e => e.eventType === 'share_revoke' && e.user === 'bob')).toBe(true);
  ...
  describe('recordAccess() / shutdown() aggregation (decision 5)', () => {
    test('aggregates hits into ONE share_access audit row on shutdown', async () => {
      const rec = await sm.issue({ kind: 'keyword', keyword: 'k' }, null, 'u');
      for (let i = 0; i < 5; i++) sm.recordAccess(rec.token);
      expect(auditEvents.filter(e => e.eventType === 'share_access')).toHaveLength(0);

      await sm.shutdown();
      const rows = auditEvents.filter(e => e.eventType === 'share_access');
