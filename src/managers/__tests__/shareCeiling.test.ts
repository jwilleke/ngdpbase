...
/** Whether jim's account is active. Flipped by a test to deactivate the issuer. */
...
function makeManager() {
  return makeDecider({
    ...
    users: (name) => (name === 'jim' ? { username: 'jim', isActive: issuerActive } : null),
    roles: () => issuerRoles
  }).pdp;
}
...
describe('#1222 — the share is the ceiling', () => {
  test('an action the share carries is allowed when the issuer holds it', async () => {
    const m = makeManager();
    expect(await m.permits(viaShare, 'page-read')).toBe(true);
    expect(await m.permits(viaShare, 'asset-read')).toBe(true);
  });
  ...
  test('an action outside the share is refused, even one the issuer holds', async () => {
    ...
    const m = makeManager();
    expect(await m.permits(viaShare, 'page-edit')).toBe(false);
  });
  ...
  test('anonymous without a share gets only what policy gives anonymous', async () => {
    ...
    const m = makeManager();
    expect(await m.permits(ANONYMOUS_SUBJECT, 'page-read')).toBe(false);
  });
});
...
describe('#1222 — the issuer\'s live authority bounds the share', () => {
  test('an issuer who lost the permission takes every share with them', async () => {
    ...
    expect(await m.permits(viaShare, 'page-read')).toBe(false);
  });
  ...
  test('an issuer who no longer exists holds nothing', async () => {
    ...
    expect(await m.permits(orphan, 'page-read')).toBe(false);
  });
  ...
  test('the share cannot carry what the record says if the issuer never held it', async () => {
    ...
    expect(await m.permits(forged, 'page-delete')).toBe(false);
  });
});
...
describe('#1222 — expiry is re-read at every decision', () => {
  test('an expired share is refused', async () => {
    ...
    expect(await m.permits(expired, 'page-read')).toBe(false);
  });
