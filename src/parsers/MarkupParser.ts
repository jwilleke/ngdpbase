...
class MarkupParser extends BaseManager {
  ...
  extractJSPWikiSyntax(content: string, _context: Record<string, unknown> = {}): { sanitized: string; jspwikiElements: ExtractedElement[]; uuid: string } {
    ...
    sanitized = sanitized.replace(new RegExp(`${UNESCAPED_BRACKET}\\[\\{(\\$\\w+)\\}\\]`, 'g'), (match: string, varName: string, offset: number) => {
      jspwikiElements.push({
        type: 'variable',
        syntax: match,
    ...
    sanitized = sanitized.replace(new RegExp(`${UNESCAPED_BRACKET}\\[\\{([A-Za-z]\\w*[^}]*)\\}\\]`, 'g'), (match: string, inner: string, offset: number) => {
      jspwikiElements.push({
        type: 'plugin',
        syntax: match,
        inner: inner.trim(),
