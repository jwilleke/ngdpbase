...
- Last worked on: #1115 — one audit vocabulary, written down and checked by a parity test. Closed #1120 #1121 #1122 #1124 the same session, finishing the audit substrate from `docs/planning/Security-auditing.md`.
- Branch / state: master, clean, 0 unpushed, 0 stashes. CI green on HEAD (3d62bffd).
- Running / in-flight: jimstest on localhost:3000, deliberately left up (the test instance). No workflows, agents, or background tasks.
- Parked / half-done: none.
- Next steps:
  - Decide `page.view`. Deliberately left out of the vocabulary — noise for a wiki, the point for the PHR direction. Needs its own issue, not an assumption. Offered, not yet filed.
  - #1123 (comments do not support NCM) and #1114 (page save slug error) are the remaining P1s; #1115 sits in review.
  - Audit scorecard in `docs/planning/Security-auditing.md` still says 8 of 12 — #1115 moved it and the doc has not been updated.
- Blockers / significant notes: jimstest has NO agent tokens minted, so the MCP write path (`ingestPageMarkdown`) has never produced an audit record on a running instance. It is proven by test only. Minting one is a live-instance action awaiting the operator's say-so.
<!-- RESUME:END -->
