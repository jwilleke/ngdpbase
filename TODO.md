...
Generated priority mirror of open GitHub issues — the `P0` / `P1` / `P2` / `deferred` labels are the source of truth. Regenerated by `/pstatus` (2026-07-23). For live state run `/pstatus`. Prior hand-curated narrative is in this file's git history.

<!-- RESUME:START -->
## ▶ Resume here — 2026-07-24

- Last worked on: #920 residual scope — #931 detection unification (#932), #930 removed-source-page policy (#934), #929 docs (#933) all merged to master; #920 + #929/#930/#931 closed.
- Branch / state: master, clean, 0 unpushed. **master is 3 commits ahead of v3.65.0 — UNRELEASED.**
- Running / in-flight: 4 pm2 servers up (jimstest:3000, fairways-base:2121, temp-build:3001, ngdpbase-veg:3333) — normal, persist across shutdown. No workflows/background agents. No open PRs. CI green.
- Parked / half-done: 1 pre-existing stash `wip: full single-evaluator refactor (#711 follow-up)` — NOT from this session, left untouched. VAAC data file untracked in geohazardwatch checkout (generated, leave).
- Next steps:
  - Release master when ready: `/semver minor` → 3.66.0 (ships #929/#930/#931), then propagate to satellites.
  - Optional: backfill `docs/performance/baseline-v3.65.0-*.md` (concurrent run skipped step 5a; v3.66 drift else compares to v3.64).
- Blockers / significant notes: **Concurrent-release hazard** — an external process (same git identity) cut v3.65.0 mid-manual-release this session (pushed tag+commit+GH release before mine). Investigate WHAT fired it (another session? scheduled /release routine?) before the next release, or two releases race again.
<!-- RESUME:END -->

## 🔴 P0 — Security & Critical

- [#922](https://github.com/jwilleke/ngdpbase/issues/922) — [bug] DataFeedPlugin link= template double-encodes full-URL placeholders, breaking generated hrefs
...
- [#915](https://github.com/jwilleke/ngdpbase/issues/915) — [FEATURE] Keyword dedup enforcement + explicit-add affordance (#869)
...
- [#917](https://github.com/jwilleke/ngdpbase/issues/917) — [FEATURE] Read keyword hierarchy into the index (keywordPaths) (#869)
- [#916](https://github.com/jwilleke/ngdpbase/issues/916) — [FEATURE] Shared keyword typeahead on the media Keywords field (#869)
