...
Generated priority mirror of open GitHub issues and pull requests — the `P0` / `P1` / `P2` / `in-review` / `deferred` labels are the source of truth. Regenerated by `/pstatus` (2026-07-27, at v3.70.3 +9). For live state run `/pstatus`. Prior hand-curated narrative is in this file's git history.

> **Security note:** the three open showdown alerts (dependabot 96, code-scanning 33/34) have **no open tracking issue** — #599 was closed after the ReDoS guard shipped. showdown is abandoned upstream, so they cannot clear by upgrading.
