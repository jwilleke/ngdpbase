...
Generated priority mirror of open GitHub issues and pull requests — the `P0` / `P1` / `P2` / `in-review` / `deferred` labels are the source of truth. Issues and PRs are ranked in the same bands by the same priority; within a band, PRs sort above issues. Regenerated by `/pstatus` (2026-08-07, at v4.5.1); hand-corrected 2026-08-10 at v4.8.4 — #1031 closed, #1039 / #1040 added, #1027 restored to P1 (it was missing entirely), and the two open `P0`s surfaced. For live state run `/pstatus`.

**Open PRs: none.**

## 🔴 P0 — Security & Critical

- [#1032](https://github.com/jwilleke/ngdpbase/issues/1032) — [SECURITY] Four untracked Dependabot alerts — *work appears complete: js-yaml fixed in v4.8.2, showdown alerts dismissed with per-CVE reasoning, repo at zero open alerts. Still labelled `P0` and open — close or move to `in-review`*
- [#1037](https://github.com/jwilleke/ngdpbase/issues/1037) — [BUG] Save-time validation lives in route handlers, so most write paths bypass it — *shipped in v4.8.3: the gate moved into `PageManager`, `collectErrors` is abstract on `BaseFilter`, orphaned `createWikiPage` deleted. Still labelled `P0` and open — close or move to `in-review`*
...
- [#1039](https://github.com/jwilleke/ngdpbase/issues/1039) — [BUG] Markdown inside `%%tabbedSection` is not processed — headings and NCM line breaks render literally — *found while finishing #1038; raw HTML works there by accident, wiki markup does not. 2 pages affected, emit path not yet traced*
- [#1027](https://github.com/jwilleke/ngdpbase/issues/1027) — [BUG] Role assignment silently does nothing when no anchor Organization is configured — *a plan exists (three-tier `getInstallOrg`, self-seeding anchor); an unfinished draft may still be sitting in `OrganizationManager.ts`*
