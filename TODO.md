...
- Last worked on: Trip Storybook shipped end-to-end — 24 day pages live on jimstest with card photo layout, one-line button nav (parser link-attribute fix 3bbc5e34), all 241 photos captioned; Dawarich Immich-compat adapter live (#864, in-review)
- Branch / state: master, clean, 0 unpushed, 1 stash (parked #711 single-evaluator refactor)
- Running / in-flight: jimstest server via pm2 (intentional); no CI/agents pending
- Parked / half-done: none
- Next steps:
  - /semver — jimstest runs ~12 unreleased commits (scan prune, Dawarich adapter, parser fixes, import fixes, media Title display)
  - #864 close check: photos on Dawarich map for a June window (its own points query is slow — narrow the range)
  - #867 close check: move a file + rescan → expect removed=1 in scan log
  - Operator: write Story prose on day pages; share link to family (#842 flow) when ready
  - Triage the #868-#877 needs-triage wave (storybook epic #871 + subs, import #874, keywords epic #869)
- Blockers / significant notes: import format dropdown resets to auto → jspwiki mangles NCM (#874); always select markdown. Live "Page Content" help page predates e4f9fca4 docs (reseed or hand-update)
<!-- RESUME:END -->
