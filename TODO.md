...
- Last worked on: AuthManager security audit (#1041–#1045 filed), then fixed #1041 (login open redirect) and #1043 (session `Secure` + fixation) together in `c3ba907f`.
- Branch / state: master, clean, nothing unpushed, no stashes.
- Running / in-flight: jimstest (3000), Fairways (2121), temp build (3001) all up on v4.8.6 + the unreleased auth fixes. No workflows, background agents or containers left running — the #1027 verification containers and volumes were removed. CI green, no open PRs.
- Parked / half-done: none.
- Next steps:
  - `/semver patch and release` — `c3ba907f` (#1041 + #1043) is committed but unreleased, so the public instances still carry the open redirect.
  - Then #1044 (no login throttle) — self-contained, and it compounds with #1042.
  - #1042 (password hashing) needs its migration designed first: versioned hashes + rehash-on-login. Do not bundle it with anything.
- Blockers / significant notes:
  - Four issues sit `in-review` waiting on the demo taking a release — #1029, #1034, #1039, #1040. Worth one eye-check pass once it does, rather than four separate ones.
  - The published GHCR image is **linux/amd64 only**, so it will not run natively on this Mac; build from the release commit when verifying container behaviour locally.
<!-- RESUME:END -->

Generated priority mirror of open GitHub issues and pull requests — the `P0` / `P1` / `P2` / `in-review` / `deferred` labels are the source of truth. Issues and PRs are ranked in the same bands by the same priority; within a band, PRs sort above issues. Regenerated by `/pstatus` (2026-08-07, at v4.5.1); hand-corrected 2026-08-11 at v4.8.6 — #1027, #1031, #1032 and #1037 closed; #1039 / #1040 fixed and in review; #1041-#1045 filed from an AuthManager audit. For live state run `/pstatus`.
...
Scanner state (2026-08-10, at v4.8.4): **ZERO open alerts, both kinds** — re-queried with `?state=open&per_page=100` on `dependabot/alerts` **and** `code-scanning/alerts`. Secret scanning is disabled on the repo. #1032 closed on that basis; #1037 closed after verifying the chokepoint in the tree (`assertContentPasses` called from both write paths, `collectErrors` abstract, `createWikiPage` gone, `block-on-save` present).
...
- [#1042](https://github.com/jwilleke/ngdpbase/issues/1042) — [BUG] Passwords stored as single-round SHA-256 — *fixed in `6c99fc3f`; scrypt + per-user salt, legacy hashes upgrade on next login. NOTE: the `amdwiki-salt` rename is still NOT safe — legacy digests depend on that exact string*
