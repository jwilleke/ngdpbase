...
Generated priority mirror of open GitHub issues and pull requests — the `P0` / `P1` / `P2` / `in-review` / `deferred` labels are the source of truth. Issues and PRs are ranked in the same bands by the same priority; within a band, PRs sort above issues. Regenerated by `/pstatus` (2026-08-07, at v4.5.1). For live state run `/pstatus`.
