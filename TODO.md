...
- Last worked on: #1101 (InterWiki `geo:`/`grok:` added, dead MeatBall + unverifiable C2 dropped) and #1102 (csv `skipLines` + preamble warning). Both shipped and pushed; CI green on `74069c38`.
- Branch / state: master, clean, nothing unpushed, no stashes.
- Running / in-flight: jimstest dev server up via `./server.sh` (pm2, PID 38789) on the current build — intentional, leave it. No workflows, background agents or in-progress CI.
- Parked / half-done: #1093 is `deferred`, NOT fixed. The CI diagnostics remain on master and keep collecting on every push (~30s/run) — remove them when it closes.
- Next steps:
  - Nothing is actionable without you: 0 P0, 0 P1, and six `in-review` items awaiting verification.
  - #1096 / #1097 need a media rescan over a folder with AVCHD files.
  - #1098 needs an undecodable video opened on its item page.
  - #1101: the `InterWikiLinks` page on jimstest still shows the OLD table — `required-pages/` seeds only on a fresh install. Edit it in the UI, or let new installs pick it up. The links themselves already work.
  - Otherwise pick a P2, or file something new.
- Blockers / significant notes: __#1093's cause was identified after it was parked__ — a bare `npm update`, not invoked by this repo, runs during the test step and rewrites `node_modules`. 14 samples on the red run, 0 on both green runs. It re-resolves against semver ranges instead of the lockfile, which explains the packages appearing that are in neither the lockfile nor a local install. The open question is now only *what spawns it on a GitHub-hosted runner and how to suppress it*. Details on the issue.
<!-- RESUME:END -->
