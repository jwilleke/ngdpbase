...
Live mirror of open GitHub issues and PRs, in the section order `/check-todos-local` reports. Labels `P0` / `P1` / `P2` / `in-review` / `deferred` are the source of truth. Regenerated 2026-07-27 at v3.70.3 +11.
...
- *(none)* — `Markdown Lint` shows historical reds from 2026-07-15 but its latest master run (2026-07-27) succeeded. `Showdown CVE-2024-1899 Patch Check` green on its weekly cron. `Docker Build and Push` green.

## 🔵 Waiting on Review Sign-off

- *(none — no open issue carries `in-review`)*
...
- **showdown alert has no tracker** — reopen #599, or file the migrate-to-`marked`/`markdown-it` issue so the alert points at intended work.
- **#999** — sidecar-only, or re-hash-and-re-key? Attachment IDs are content hashes, so an embedded write breaks them.
- **#868 / #550** — whether `es-enrichment-pipeline` survives; both rest on it.

## ✅ Easy wins

- [#963](https://github.com/jwilleke/ngdpbase/issues/963) — DataFeedPlugin empty state. Replace one debug-looking string, add an optional `empty=` override. Single acceptance criterion, additive.
- [#969](https://github.com/jwilleke/ngdpbase/issues/969) — admin trash view. The API already exists (`/api/admin/deleted-pages`), so this is one view over a proven surface.

Only two qualify — most of the P2 band is addon proposals or storybook sub-issues of EPIC #871, multi-slice by definition.
...
1. **Settle the showdown alert** — reopen #599 or file the migration issue. Third consecutive survey reporting an untracked open alert.
2. **#898** — storybook orphans a `route.png` on every regeneration; it accumulates, same shape as #970.
3. **#963** — smallest real win on the board.
