...
- [#1087](https://github.com/jwilleke/ngdpbase/issues/1087) — [BUG] Headless install docs claim it refuses to start without NGDPBASE_ADMIN_PASSWORD — it boots with admin123
- [#1086](https://github.com/jwilleke/ngdpbase/issues/1086) — [BUG] POST /register leaks every username on the instance to an unauthenticated visitor
- [#1090](https://github.com/jwilleke/ngdpbase/issues/1090) — [BUG] ngdpbase.server.port is not the port the server binds to — app.listen reads process.env.PORT only
- [#1089](https://github.com/jwilleke/ngdpbase/issues/1089) — [BUG] /admin/configuration accepts and persists edits to six env-overridden keys that can never take effect
...
- [#1091](https://github.com/jwilleke/ngdpbase/issues/1091) — [BUG] migrate-to-versioning.ts resolves pages to a literal ${SLOW_STORAGE} dir — no .env load, no env-ref resolution
- [#1088](https://github.com/jwilleke/ngdpbase/issues/1088) — [FEATURE] Remove the dead hand-rolled .env parser in app.ts — bootstrap-env.ts already did the work
